...
describe('parseColor', function(): void {
  ...
  describe('color objects', function(): void {
    const inputs: { name: string; color: ColorObject; defaultColor?: FullColorObject; expected: ColorObject }[] = [
      {
    ...
    inputs.forEach(({ name, color, defaultColor, expected }): void => {
      it(name, function(): void {
        expect(parseColor(color, defaultColor as FullColorObject)).to.be.deep.equal(expected)
      })
    })
  })
})
